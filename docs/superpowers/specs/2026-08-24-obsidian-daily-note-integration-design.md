...
| 要素 | 出力 |
|---|---|
| 見出し | 今日のメモなら `## HH:MM {title}`、今日でなければ `## YYYY-MM-DD HH:MM {title}` — `obsidian://daily` は常に「今日」のノートに追記するため、過去のメモだといつのものか判別できなくなる。時刻は `memo.ts` のローカル時刻、`title` が空なら `音声メモ` |
...
1. 組み立てた URI 全体の長さが `OBSIDIAN_URL_LIMIT`（既定 8000 文字）以下なら、`content=` で送る

   日本語1文字は `encodeURIComponent` で `%XX%XX%XX` の9文字になる。実測で Markdown 117文字のメモが URI 678文字（約5.8倍）だったため、当初の 2000 だと約210文字で上限に達し、二段構えが「ほぼ常に clipboard」になってしまうことが最終レビューで判明した。
...
送信済みリストはバックアップ（書き出し）にも含める（バックアップ形式 v3）。含めないと端末移行後に全メモが未送信扱いになり、確認なしで再送されて重複追記されるため。

また、メモに「追記」したときは内容が変わるので、そのメモの送信済みマークを外す（`unmarkObsidianSent()`）。
...
| `OBSIDIAN_URL_LIMIT` | 8000 | 実機検証の結果 |
